...
//                d_data->density_sph[i] = d_const->rest_density.x;
//                break;
//            case Phase::PHASE2:
//                d_data->volF_k[i] = {0, 1};
//                d_data->density_m[i] = d_const->rest_density.y;
//                d_data->density_sph[i] = d_const->rest_density.y;
//                break;
//            case Phase::PHASE3:
//                // TODO
//                break;
...
//        d_data->mass_m[i] = d_data->density_m[i] * d_const->rest_volume;
//
//    }
//
//    __global__ void
...
//        float sigma = 1e-4;
//
//        // compute gradient of pressure_m and volume fraction
//        float3 dp_k1 = {0, 0, 0};
//        float3 dp_k2 = {0, 0, 0};
//        float3 da_k1 = {0, 0, 0};
//        float3 da_k2 = {0, 0, 0};
...
//                continue;
//
//            dp_k1 += d_data->density_m[p_j] * d_const->rest_volume /
//                     d_data->density_sph[p_j] * (d_data->pressure_k[p_j].x - d_data->pressure_k[p_i].x) *
//                     cubic_grad(pos_i - pos_j, d_const->sph_h);
//
//            dp_k2 += d_data->density_m[p_j] * d_const->rest_volume /
//                     d_data->density_sph[p_j] * (d_data->pressure_k[p_j].y - d_data->pressure_k[p_i].y) *
//                     cubic_grad(pos_i - pos_j, d_const->sph_h);
//
//            da_k1 += d_data->density_m[p_j] * d_const->rest_volume /
//                     d_data->density_sph[p_j] * (d_data->volF_k[p_j].x - d_data->volF_k[p_i].x) *
//                     cubic_grad(pos_i - pos_j, d_const->sph_h);
//
//            da_k2 += d_data->density_m[p_j] * d_const->rest_volume /
//                     d_data->density_sph[p_j] * (d_data->volF_k[p_j].y - d_data->volF_k[p_i].y) *
//                     cubic_grad(pos_i - pos_j, d_const->sph_h);
//        }
//
//        // 3 sum-items in eq(9)
...
//            auto pos_j = d_data->predictPos[p_j];
//
//            float ck1 = d_data->volF_k[p_i].x * d_const->rest_density.x / d_data->density_m[p_i];
//            float ck2 = d_data->volF_k[p_i].y * d_const->rest_density.y / d_data->density_m[p_i];
//
...
//
//            dp_m += d_data->density_m[p_j] * d_const->rest_volume *
//                    (d_data->pressure_m[p_i] + d_data->pressure_m[p_j]) / 2 *
//                    gradW;
//
//            acc += d_data->density_m[p_j] * d_const->rest_volume / d_data->density_sph[p_j] * niu *
//                   (d_data->v_m[p_j] - d_data->v_m[p_i]) *
//                   dot(-pos_ij, gradW) / (length(pos_ij) * length(pos_ij));
//        }
//        acc = dp_m / d_data->density_m[p_i] - acc;
//
//        // compute v_k, eq(9)
...
//            float f1 = d_data->density_m[p_j] / d_data->density_sph[p_j] * d_const->rest_volume;
...
//
//        if (d_data->mat[p_i] != Material::BOUND)
//            d_data->density_m[p_i] = dot(d_data->volF_k[p_i], d_const->rest_density);
//        d_data->mass_m[p_i] = d_data->density_m[p_i] * d_const->rest_volume;
//        d_data->density_sph[p_i] = max(d_data->density_m[p_i], d_data->density_sph[p_i]);
//        float k = 220 * 220;
//        d_data->pressure_m[p_i] =
//                k * d_data->density_m[p_i] / 7 * (pow(d_data->density_sph[p_i] / d_data->density_m[p_i], 7) - 1);
//
//        if (d_data->pressure_m[p_i] < 1e-6)
//            d_data->pressure_m[p_i] = 0;
//
//        if (d_data->mat[p_i] == Material::BOUND)
//            d_data->pressure_k[p_i] = d_data->pressure_m[p_i] * float2{0.5, 0.5};
//        d_data->pressure_k[p_i] = d_data->pressure_m[p_i] * d_data->volF_k[p_i];
//    }
//
...
//
//            // for phase1
//            d1 += -k * d_const->rest_density.x / 7 *
//                  (6 * pow(d_data->density_sph[p_i] / d_data->density_m[p_i], 7) + 1) *
//                  d_data->d_volF[p_i].x;
//
//            // for phase2
//            d2 += -k * d_const->rest_density.y / 7 *
//                  (6 * pow(d_data->density_sph[p_i] / d_data->density_m[p_i], 7) + 1) *
//                  d_data->d_volF[p_i].y;
//
//        }
...
//
//            dp_m += d_data->density_m[p_j] * d_const->rest_volume *
//                    (d_data->pressure_m[p_i] + d_data->pressure_m[p_j]) /
//                    (2 * d_data->density_sph[p_j]) * gradW;
//
////            if (d_data->mat[p_j] != Material::BOUND)
////                vis_acc += d_data->density_m[p_j] * d_const->rest_volume / d_data->density_sph[p_j] * niu *
////                           (d_data->v_m[p_j] - d_data->v_m[p_i]) *
////                           dot(-pos_ij, gradW) / pow(length(pos_ij), 2);
//        }
//
//        d_data->acc_m[p_i] = -dp_m / d_data->density_m[p_i] + vis_acc / d_data->density_m[p_i] + d_const->gravity;
//
//    }
//
//    __global__ void
...
//            return;
//
//        d_data->v_m[p_i] += d_data->acc_m[p_i] * d_const->dt;
//        d_data->predictPos[p_i] += d_data->v_m[p_i] * d_const->dt;
//        d_data->pos[p_i] = d_data->predictPos[p_i];
//    }
//
//    __global__ void
//    estimate_density_cuda(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors) {
//        uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
//
//        if (i >= d_const->total_particle_num)
//            return;
...
//
//            auto pos_j = d_data->predictPos[p_j];
//            d_data->density_sph[p_i] += d_data->mass_m[p_j] * cubic_value(pos_i - pos_j, d_const->sph_h);
//        }
//
//        __syncthreads();
//
//        d_data->density_m[p_i] = dot(d_const->rest_density, d_data->volF_k[p_i]);
//        d_data->mass_m[p_i] = d_data->density_m[p_i] * d_const->rest_volume;
//        d_data->pressure_m[p_i] = 500000 * d_data->density_m[p_i] / 7 *
...
//
//    __global__ void
//    predict_vel_cuda(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors) {
//        uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
//
//        if (i >= d_const->total_particle_num)
//            return;
...
//        auto pos_i = d_data->predictPos[p_i];
//
//        d_data->acc_m[p_i] = d_const->gravity + d_data->M_m[p_i];
//        d_data->v_m[p_i] += d_data->acc_m[p_i] * d_const->dt;
//        __syncthreads();
//
//        // compute wc pressure acc
//        float3 p_acc = {0, 0, 0};
//        float gamma = d_data->volF_k[p_i].x * d_data->density_m[p_i] / d_const->rest_density.x +
//                      d_data->volF_k[p_i].y * d_data->density_m[p_i] / d_const->rest_density.y;
//        for (uint32_t nb_0 = p_i * d_const->ns_maxNeighborNum, t = 0, p_j = neighbors[nb_0 + t];
//             t < d_const->ns_maxNeighborNum && p_j != UINT_MAX; ++t, p_j = neighbors[nb_0 + t]) {
//
//            auto pos_j = d_data->predictPos[p_j];
//
//            p_acc += -gamma / d_data->density_m[p_i] * d_data->mass_m[p_j] *
//                     (d_data->pressure_m[p_j] / pow(d_data->density_m[p_j], 2) +
//                      d_data->pressure_m[p_i] / pow(d_data->density_m[p_i], 2)) *
//                     cubic_grad(pos_i - pos_j, d_const->sph_h);
//
//        }
//        d_data->v_m[p_i] += p_acc * d_const->dt;
////
////        // compute vis acc, tao_m, tao_tm, tao_dm
////        d_data->density_sph[p_i] *= 0;
////        float3 tao_acc = {0, 0, 0};
////        for (uint32_t nb_0 = p_i * d_const->ns_maxNeighborNum, t = 0, p_j = neighbors[nb_0 + t];
////             t < d_const->ns_maxNeighborNum && p_j != UINT_MAX; ++t, p_j = neighbors[nb_0 + t]) {
////
////            // TODO
...
////        }
////
////        d_data->acc_m[p_i] += tao_acc;
//
//    }
//
//    __global__ void
//    advect_pos_cuda(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors) {
//        uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
//
//        if (i >= d_const->total_particle_num)
//            return;
...
//
//        // TODO 先让相2的流体不动作边界
//        if (d_data->original_phase[p_i] == Phase::PHASE2)
//            return;
//
//        d_data->predictPos[p_i] += d_data->v_m[p_i] * d_const->dt;
//        d_data->pos[p_i] = d_data->predictPos[p_i];
...
//
//    __global__ void
//    phase_diffusion_cuda(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors) {
//        uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
//
//        if (i >= d_const->total_particle_num)
//            return;
...
//
//    __host__ void
//    estimate_density(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors,
//                     uint32_t blockNum, uint32_t threadNum) {
//        estimate_density_cuda<<< blockNum, threadNum>>>(d_const, d_data, partIndex, neighbors);
//    }
//
//    __host__ void
//    predict_vel(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors,
//                uint32_t blockNum, uint32_t threadNum) {
//        predict_vel_cuda<<< blockNum, threadNum>>>(d_const, d_data, partIndex, neighbors);
//    }
//
//    __host__ void
//    advect_pos(ConstParams *d_const, DynamicParams *d_data, uint32_t *partIndex, uint32_t *neighbors,
//               uint32_t blockNum, uint32_t threadNum) {
//        advect_pos_cuda<<< blockNum, threadNum>>>(d_const, d_data, partIndex, neighbors);
//    }
//
//    __host__ void
